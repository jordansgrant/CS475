...
const char *			CL_FILE_NAME = { "prog6.cl" };
...
main( int argc, char *argv[ ] )
{
  ...
  float *hZ = new float[ NUM_WORK_GROUPS ];
  ...
  status = clEnqueueWriteBuffer( cmdQueue, dZ, CL_FALSE, 0, dataSizeWG, hZ, 0, NULL, NULL );
  if( status != CL_SUCCESS )
      print_cl_status(status);
  ...
  for (int i = 0; i < 15; i++) {
    ...
    double perf = (double)NUM_ELEMENTS/(time1-time0)/1000000.0;
    //fprintf(stderr, "%10.3lf\n", perf);  
    if (perf > max_perf)
      max_perf = perf;
    sum_perf += perf;
  }
  fprintf( stderr, "Num Elements %8d\tLocal Size %4d\tWork Groups %10d\nAvg Perf %10.3lf MegaMultsPerSec\nMax Perf %10.3lf MegaMultsPerSec\n\n",
      NUM_ELEMENTS, LOCAL_SIZE, NUM_WORK_GROUPS, sum_perf / 15, max_perf );
  printf("%10.3lf", max_perf);
